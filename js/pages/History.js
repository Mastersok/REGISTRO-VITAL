...
window.Pages.HistoryView = (router) => {
    ...
    const filterOptions = [
        { id: 'pressure', label: t('pressure_short'), icon: 'favorite' },
        { id: 'glucose', label: t('glucose'), icon: 'bloodtype' },
        { id: 'oxygen_temp', label: t('bio_health_short'), icon: 'thermostat' },
        ...
        let html = `
            ...
            <div class="grid grid-cols-4 gap-2">
                <button onclick="window.toggleCategoryFilter('all')" 
                    ...
                    <span class="text-[9px] font-black uppercase tracking-tighter">${t('all')}</span>
                </button>
    ...
    const updateHeader = () => {
        ...
        if (currentDateFilter) {
            const locale = window.DosisStore.state.settings.language === 'en' ? 'en-US' : 'es-ES';
            dateEl.innerHTML = `<span class="text-primary font-bold">📅 ${currentDateFilter.toLocaleDateString(locale, { day: 'numeric', month: 'short' })}</span>`;
            filterBtn.classList.remove('bg-white', 'dark:bg-slate-800');
            filterBtn.classList.add('bg-primary', 'text-white');
            titleEl.innerText = t('filter').toUpperCase();
        } else {
            dateEl.innerHTML = t('history_full').toUpperCase();
            ...
            titleEl.innerText = t('history').toUpperCase();
        }
        renderFilterBar();
    };
    ...
    const renderList = () => {
        ...
        if (countEl) {
            const countLabel = readings.length === 1 ? t('record') : t('records');
            countEl.innerText = `${readings.length} ${countLabel}`;
        ...
        if (readings.length === 0) {
            ...
            mainEl.innerHTML = `
                <div class="flex flex-col items-center justify-center pt-20 opacity-50 space-y-4 animate-up">
                    ...
                    <p class="text-xl font-bold text-gray-400 dark:text-slate-600">${t('no_results')}</p>
                    ${hasFilters ? `<button onclick="window.resetFilter()" class="h-12 px-6 bg-primary/10 text-primary font-black rounded-xl active:scale-95 transition-all text-[10px] uppercase">${t('clean_filters')}</button>` : ''}
                </div>
            `;
            updateHeader();
            return;
        ...
        readings.forEach(item => {
            ...
            const dayStr = date.toLocaleDateString(window.DosisStore.state.settings.language === 'en' ? 'en-US' : 'es-ES', { weekday: 'long', day: 'numeric', month: 'long' });
            ...
            const typeConfig = {
                pressure: { label: t('pressure'), getVal: v => `${v.systolic}/${v.diastolic}`, unit: 'mmHg', sub: v => `${t('pulse')}: ${v.pulse}` },
                glucose: { label: t('glucose'), getVal: v => v.value, unit: 'mg/dL', sub: v => item.timing },
                oxygen_temp: {
                    label: t('bio_health'),
                    getVal: v => v.spo2 ? `${v.spo2}%` : (v.temp ? `${v.temp}°C` : '---'),
                    unit: v => v.spo2 ? 'SpO2' : (v.temp ? 'TEMP' : ''),
                    sub: v => (v.spo2 && v.temp) ? `Temp: ${v.temp}°C` : (v.spo2 ? t('status_low') : t('status_low'))
                },
            ...
            try {
                ...
                readingListHtml += `
                    <div class="relative mb-4 group touch-pan-y select-none" id="reading-${item.id}">
                        ...
                        <div class="swipe-foreground relative bg-white dark:bg-slate-800 p-6 rounded-[2.5rem] border border-gray-100 dark:border-slate-700 shadow-premium flex items-center gap-6 z-10 transition-transform duration-300 ease-[cubic-bezier(0.2,0,0,1)] cursor-grab active:cursor-grabbing"
                            ...
                            <div class="size-14 rounded-2xl ${colors[semaphore] || colors.normal} flex items-center justify-center text-white shadow-lg shrink-0 pointer-events-none">
                                <span class="material-symbols-outlined !text-2xl">${icons[item.type] || 'help'}</span>
                            </div>
                ...
                console.error("Error rendering reading:", err);
            }
        });
    ...
    el.innerHTML = `
        <header class="p-6 pb-4 sticky top-0 bg-gray-50/90 dark:bg-slate-900/90 backdrop-blur-xl z-40 transition-all duration-300 border-b border-gray-100 dark:border-slate-800 shadow-sm">
            ...
            <div class="flex flex-col mt-2">
                <h2 class="text-3xl font-black italic tracking-tighter transition-all dark:text-white">${t('history').toUpperCase()}</h2>
                <p id="header-date" class="text-[9px] font-black tracking-[0.3em] text-gray-400 dark:text-slate-500 mt-0.5 uppercase transition-all">${t('history_full').toUpperCase()}</p>
            </div>
    ...
    window.deleteItem = (id) => { if (confirm(t('confirm_delete'))) { window.DosisStore.deleteReading(id); renderList(); } else { const el = document.querySelector(`#reading-${id} > .swipe-foreground`); if (el) el.style.transform = 'translateX(0)'; window.currentSwipedId = null; } };
    window.editItem = (id) => {
